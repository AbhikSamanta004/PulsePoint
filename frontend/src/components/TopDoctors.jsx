    ...
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 mdLmx-10'>
      ...
      <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {doctors.slice(0, 10).map((item, index) => (
          <div onClick={() => { navigate(`/appointment/${item._id}`); scrollTo(0, 0) }} className='border border-border-color rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500 hover:shadow-card-hover' key={index}>
      ...
      <button onClick={() => { navigate('/doctors'); scrollTo(0, 0) }} className='bg-light-tint text-primary px-12 py-3 rounded-full mt-10 hover:bg-primary hover:text-white transition-all duration-300'>more</button>
    </div>
  )
}
