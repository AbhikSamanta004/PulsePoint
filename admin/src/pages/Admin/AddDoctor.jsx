...
const AddDoctor = () => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className="m-5 w-full">
      <p className="mb-3 text-lg font-medium text-heading">Add Doctor</p>
      <div className="bg-white px-8 py-8 border border-border-color rounded-xl w-full max-w-4xl max-h-[80vh] overflow-y-scroll shadow-sm">
        <div className="flex items-center gap-4 mb-8 text-body">
          <label htmlFor="doc-img">
            <img className="w-16 bg-light-tint rounded-full cursor-pointer p-1 border-2 border-dashed border-primary/30 hover:border-primary transition-all" src={docImg ? URL.createObjectURL(docImg) : assets.upload_area} alt="" />
          </label>
        ...
        <div className="flex flex-col lg:flex-row items-start gap-10 text-heading">
          <div className="w-full lg:flex-1 flex flex-col gap-4">
            ...
            <div className="flex-1 flex flex-col gap-1">
              ...
              <input onChange={(e) => setEmail(e.target.value)} value={email} className="border border-border-color rounded px-3 py-2 focus:border-primary outline-none transition-all" type="email" placeholder="Email" required />
            </div>

            <div className="flex-1 flex flex-col gap-1">
              <p className="font-medium">Doctor Password</p>
              <input onChange={(e) => setPassword(e.target.value)} value={password} className="border border-border-color rounded px-3 py-2 focus:border-primary outline-none transition-all" type="password" placeholder="Password" required />
            </div>

            <div className="flex-1 flex flex-col gap-1">
              <p className="font-medium">Experience</p>
              <select onChange={(e) => setExperience(e.target.value)} value={experience} className="border border-border-color rounded px-3 py-2 focus:border-primary outline-none transition-all" name="" id="">
          ...
          <div className="w-full lg:flex-1 flex flex-col gap-4 text-heading">
            <div className="flex-1 flex flex-col gap-1">
              ...
              <select onChange={(e) => setSpeciality(e.target.value)} value={speciality} className="border border-border-color rounded px-3 py-2 focus:border-primary outline-none transition-all" name="" id="">
        ...
        <button type="submit" className="bg-primary hover:bg-primary-hover px-10 py-3 mt-4 text-white rounded-full font-medium transition-all shadow-md">Add Doctor</button>
      </div>
